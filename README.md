...
- RC失效或连续50 ms无新数据时终止动作，并要求恢复后先回中；
...
- RC输入无效或连续50 ms无新数据：最迟在下一次FastLoop输出检查时令M5、M6均停止；
- 180 ms单步运行期间，RC9回中或换向不会截短当前动作，也不会排队下一步；动作结束后必须重新采到中位，才允许下一次触发；
...
| 构建 | 结果 | Flash使用/剩余 |
|---|---|---|
| `MatekF405-UAV` ArduCopter（单步180 ms，安全复核修正版） | 通过 | ELF负载899,787 B / 剩余83,252 B（BIN为899,788 B） |
| 标准 `MatekF405` ArduCopter（隔离对照） | 通过，未包含云台提示字符串 | 897,943 B / 85,092 B |
...
- 待测：上电不动作、一次离中只运行一步、持续偏杆不重复、动作结束后回中可重触发、RC失效或50 ms无新数据后停止、M5/M6严格互斥；
